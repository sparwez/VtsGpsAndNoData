...
function write_datagap_report_xml($vserial, $startdate, $enddate, $skip_nogps_interval, $skip_nodata_interval, $xmltowrite,$root)
{
	...
	for($i=0;$i<sizeof($vserial);$i++)
	{
		...
		get_detail_nogps($vserial[$i], $vname[$i], $startdate, $enddate, $skip_nogps_interval,$skip_nodata_interval);
  ...
  /////////// NO GPS OPENS ///////////////////////  
  function get_detail_nogps($imei, $vname, $startdate, $enddate, $skip_interval,$skip_nodata_interval)
  {   
  	//echo '<br>in No data';
					...
					if($xml_date!=null)
					{
						if(($xml_date >= $startdate && $xml_date <= $enddate) && ($xml_date!="-"))
						{
							...
							else
							{
								...
								//echo "xml_date=".$xml_date."prev_xml_date=".$prev_xml_date."tmp_diff=".$tmp_diff."<br>";
